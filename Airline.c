...
int		initAirlineFromFile(Airline* pComp, AirportManager* pManaer, const char* fileName, int compress)
{
	...
	if (!loadAirlineFromCompressedFile(pComp, pManaer, fileName))
		return 0;
	return initDateList(pComp);
}

//function reads from compressed file
int loadAirlineFromCompressedFile(Airline* pComp, AirportManager* pManager, const char* fileName)
{
	FILE* fp;
	...
	pComp->flightCount = (data[0] << 1) | ((data[1] >> 7) & 0x1); //read flight count
	pComp->sortOpt = (eSortOption)((data[1] >> 4) & 0x7); //read sort option
	int len = data[1] & 0xF; //read name length
	pComp->name = (char*)calloc(len + 1, sizeof(char));
	if (!pComp->name)
		return 0;

	if (fread(pComp->name, sizeof(char), len, fp) != len) // read name
	{
		free(pComp->name);
		return 0;
	}
	...
	if (!(loadFlightArrayFromCompressedFile(pComp, pManager, fp)))
	{
		free(pComp->name);
		fclose(fp);
		return 0;
...
//function loads flights to flight array from compressed file
int loadFlightArrayFromCompressedFile(Airline* pComp, AirportManager* pManager, FILE* fp)
{
	...
	pComp->flightArr = (Flight**)malloc(pComp->flightCount * sizeof(Flight*)); //allocate space for flight array
	if (!pComp->flightArr)
	{
	...
	for (int i = 0; i < pComp->flightCount; i++)
	{
		...
		if (!loadFlightFromCompressedFile(pComp->flightArr[i], pManager, fp))
		{
			free(pComp->flightArr);
			return 0;
		}
...
//function saves airline to compressed file
int saveAirlineToCompressedFile(const Airline* pComp, const char* fileName)
{
	FILE* fp;
	fp = fopen(fileName, "wb");
	if (!fp) {
	...
	data[1] = (pComp->flightCount & 0x1) << 7;
	data[1] = data[1] | pComp->sortOpt << 4;
	data[1] = data[1] | len;
	...
	if (!saveFlightsArrayToCompressedFile(pComp, fp))
	{
		fclose(fp);
		return 0;
	}
...
//function saves the flight array into a compressed file 
int saveFlightsArrayToCompressedFile(const Airline* pComp, FILE* fp)
{
	for (int i = 0; i < pComp->flightCount; i++)
	{
		if (!saveFlightToCompressedFile(pComp->flightArr[i], fp))
			return 0;
	}
	return 1;
}
